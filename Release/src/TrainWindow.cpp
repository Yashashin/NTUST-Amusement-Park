...
//========================================================================
{
	...
	begin();	// add to this widget
	{
		...
		topCam = new Fl_Button(735, pty, 60, 20, "TopCam");
        topCam->type(FL_RADIO_BUTTON);
        topCam->value(0);
        topCam->selection_color((Fl_Color)3);
		topCam->callback((Fl_Callback*)damageCB,this);
		pty += 30;
	    ferrisWheelCam = new Fl_Button(605, pty, 120, 20, "FerrisWheelCam");
		ferrisWheelCam->type(FL_RADIO_BUTTON);		// radio button
		ferrisWheelCam->value(0);			// turned on
		ferrisWheelCam->selection_color((Fl_Color)3); // yellow when pressed
		ferrisWheelCam->callback((Fl_Callback*)damageCB, this);
		cupCam = new Fl_Button(735, pty, 60, 20, "CupCam");
		cupCam->type(FL_RADIO_BUTTON);		// radio button
		cupCam->value(0);			// turned on
		cupCam->selection_color((Fl_Color)3); // yellow when pressed
		cupCam->callback((Fl_Callback*)damageCB, this);
		camGroup->end();
		...
		cameraBrowser = new Fl_Browser(605, pty, 120, 120, "Camera Type");
		cameraBrowser->type(2);  // select
		cameraBrowser->callback((Fl_Callback*)damageCB, this);
		cameraBrowser->add("World");
		cameraBrowser->add("Train");
		cameraBrowser->add("Top");
		cameraBrowser->add("FerrisWheel1");
		cameraBrowser->add("FerrisWheel2");
		cameraBrowser->add("Cup");
		cameraBrowser->select(1);


		pty += 140;
		Fl_Group* skys = new Fl_Group(600, pty, 195, 20);
		skys->begin();

		stars = new Fl_Button(605, pty, 60, 20, "Stars");
		stars->type(FL_RADIO_BUTTON);  // radio button
		stars->value(1);   // turned on
		stars->selection_color((Fl_Color)3); // yellow when pressed
		stars->callback((Fl_Callback*)damageCB, this);
		white = new Fl_Button(670, pty, 80, 20, "Azure sky");
		white->type(FL_RADIO_BUTTON);
		white->value(0);
		white->selection_color((Fl_Color)3);
		white->callback((Fl_Callback*)damageCB, this);
		skys->end();

		pty += 30;
		particleType = new Fl_Value_Slider(655, pty, 140, 20, "Firework");
		particleType->range(0, 3);
		particleType->value(0);
		particleType->align(FL_ALIGN_LEFT);
		particleType->type(FL_HORIZONTAL);
...
//========================================================================
{
	...
	//#####################################################################
	trainView->time += speed->value();
	trainView->count_height_map++;
	if (trainView->count_height_map > 199)
	{
		trainView->count_height_map -= 200;
	}
	trainView->trainU += (trainView->trainAcc + speed->value());
	if (trainView->trainU >= trainView->totalArc)
	{
		trainView->trainU -= trainView->totalArc;
	}
